...
async function renderAdminPanel() {
  ...
  MembersDB.exportJSON().then((j) => { cachedExportJSON = j; });
  ...
  document.getElementById('shareBtn').addEventListener('click', () => {
    const attemptShare = (json) => {
      ...
      // Some mobile browsers (notably Android Chrome) don't whitelist
      // application/json for file sharing and reject/refuse silently.
      // Retry with text/plain — still the same actual .json file/content,
      // just a MIME type more broadly accepted by the share sheet.
    ...
    if (cachedExportJSON !== null) {
      ...
      MembersDB.exportJSON().then(attemptShare);
    }
  });
