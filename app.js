...
async function router() {
  ...
  if (segments[0] === 'filter') return renderFilterPage(params);
  ...
  APP_ROOT.innerHTML = `
    <div class="container">
      ...
      <div class="nav-grid">
        ...
        <a href="#/filter" class="nav-card">
          <span class="icon-wrap">${ICONS.search}</span>
          <span>تصفية</span>
        </a>
      </div>
    </div>
...
async function renderBrowse(key, valueRaw, params) {
  ...
  APP_ROOT.innerHTML = `
    <div class="container">
      ...
      <p class="section-sub">${members.length} اسم مسجَّل</p>
...
/* ---------------------------------------------------------------------- */
/*  Filter page: multi-field AND filter over existing data (read-only)    */
/* ---------------------------------------------------------------------- */
async function renderFilterPage(params) {
  renderChrome(true);
  const f = {
    city: params.city || '',
  ...
  const sectorOptions = f.stage && SECTOR_MAP[f.stage] ? SECTOR_MAP[f.stage] : [];
  const classOptions = f.sector && CLASS_MAP[f.sector] ? CLASS_MAP[f.sector] : [];

  const hasFilter = f.city || f.neighborhood || f.stage || f.sector || f.class;
  let results = [];
  if (hasFilter) {
    const all = await MembersDB.getAll();
    results = all.filter((m) =>
      (!f.city || m.city === f.city) &&
      (!f.neighborhood || m.neighborhood === f.neighborhood) &&
      (!f.stage || m.stage === f.stage) &&
      (!f.sector || m.sector === f.sector) &&
      (!f.class || m.class === f.class)
    );
  }

  // Breakdown by sector among the filtered results (only meaningful with results)
  const breakdown = {};
  results.forEach((m) => {
    const k = cleanLabel(m.sector || 'غير محدد');
    breakdown[k] = (breakdown[k] || 0) + 1;
  });

  APP_ROOT.innerHTML = `
    <div class="container">
      <p class="breadcrumbs"><a href="#/">الرئيسية</a><span class="sep">/</span><span>تصفية</span></p>
      <h2 class="section-title">تصفية البيانات</h2>
      <form id="filterForm" class="form-card">
        <div class="form-grid">
          ${dependentSelectHTML('fcity', 'المدينة', CITY_OPTIONS, f.city, false)}
          ${dependentSelectHTML('fneighborhood', 'الحي', NEIGHBORHOOD_OPTIONS, f.neighborhood, false)}
          ${dependentSelectHTML('fstage', 'المرحلة', STAGE_OPTIONS, f.stage, false)}
          ${dependentSelectHTML('fsector', 'القطاع', sectorOptions, f.sector, !f.stage)}
          ${dependentSelectHTML('fclass', 'الفصل', classOptions, f.class, !f.sector)}
        </div>
        <div class="admin-actions">
          <button type="submit" class="btn btn-primary">تصفية</button>
          <button type="button" id="clearFilterBtn" class="btn btn-outline">مسح التصفية</button>
        </div>
      </form>

      ${hasFilter ? `
        <p class="section-sub">إجمالي المسجلين: ${results.length}</p>
        ${Object.keys(breakdown).length ? `<div class="chip-row">${Object.entries(breakdown).map(([k, c]) => `<span class="chip">${escapeHTML(k)}<span class="count">${c}</span></span>`).join('')}</div>` : ''}
        ${renderMemberListOrEmpty(results, 'لا يوجد أسماء مطابقة لمعايير التصفية')}
      ` : ''}
    </div>`;

  const fstageSelect = document.getElementById('f_fstage');
  const fsectorSelect = document.getElementById('f_fsector');
  const fclassSelect = document.getElementById('f_fclass');

  function fillSelectLocal(select, options, selectedValue) {
    select.innerHTML = '<option value="">— اختر —</option>' + optionsHTML(options, selectedValue);
  }

  fstageSelect.addEventListener('change', () => {
    fillSelectLocal(fsectorSelect, SECTOR_MAP[fstageSelect.value] || [], '');
    fsectorSelect.disabled = !fstageSelect.value;
    fillSelectLocal(fclassSelect, [], '');
    fclassSelect.disabled = true;
  });

  fsectorSelect.addEventListener('change', () => {
    fillSelectLocal(fclassSelect, CLASS_MAP[fsectorSelect.value] || [], '');
    fclassSelect.disabled = !fsectorSelect.value;
  });

  document.getElementById('filterForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const next = {
      city: document.getElementById('f_fcity').value,
      neighborhood: document.getElementById('f_fneighborhood').value,
      stage: fstageSelect.value,
      sector: fsectorSelect.value,
      class: fclassSelect.value,
    };
    navigate(`/filter?${qs(next)}`);
  });
...
const STAGE_OPTIONS = [
  'مدارس_الأحد',
  'اعدادي بنين', 'اعدادي بنات',
  'ثانوي بنين', 'ثانوي بنات',
  'جامعة شباب', 'جامعة شابات',
  'خريجين شباب', 'خريجين شابات',
];

/* المرحلة -> القطاع */
const SECTOR_MAP = {
  'مدارس_الأحد': ['حضانه', 'ابتدائي_أ', 'ابتدائي_ب'],
  'اعدادي بنين': ['اعدادي'],
  'اعدادي بنات': ['اعدادي'],
  'ثانوي بنين': ['ثانوي'],
  'ثانوي بنات': ['ثانوي'],
  'جامعة شباب': ['اجتماع الشباب'],
  'جامعة شابات': ['اجتماع الشباب'],
  'خريجين شباب': ['اجتماع الخريجين'],
  'خريجين شابات': ['اجتماع الخريجين'],
};
...
const CLASS_MAP = {
  ...
  'اعدادي': ['أولي إعدادي', 'ثانية إعدادي', 'ثالثة إعددي'],
  'ثانوي': ['أولي ثانوي', 'ثانية ثانوي', 'ثالثة ثانوي'],
  'اجتماع الشباب': ['اجتماع الشباب والشابات'],
  'اجتماع الخريجين': ['اجتماع الشباب والشابات'],
};
...
async function renderForm(idStr) {
  ...
  document.getElementById('memberForm').addEventListener('submit', async (e) => {
    ...
    const record = {
      ...
      stageGender: deriveStageGender(document.getElementById('f_stage').value.trim()) || (v.stageGender || null),
...
async function renderAdminPanel() {
    ...
    <div class="container">
      ...
      <div class="admin-panel">
        ...
        <div class="admin-actions">
          ...
          <button id="shareBtn" class="btn" style="background:#25D366;color:#fff;">${ICONS.download}<span>مشاركة على الواتساب</span></button>
        </div>
      </div>
  ...
  document.getElementById('shareBtn').addEventListener('click', async () => {
    const json = await MembersDB.exportJSON();
    const stamp = new Date().toISOString().slice(0, 10);
    const file = new File([json], `church-members-backup-${stamp}.json`, { type: 'application/json' });
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: 'نسخة بيانات الكنيسة' });
      } catch (err) {
        if (err && err.name !== 'AbortError') showToast('تعذّرت المشاركة', 'danger');
      }
    } else {
      showToast('مشاركة الملفات غير مدعومة على هذا المتصفح/الجهاز — استخدم زر "تنزيل نسخة JSON" ثم شاركه يدويًا', 'danger');
    }
  });
