...
const STAGE_OPTIONS = [
  ...
  'أولي ابتدائي', 'ثانية ابتدائي', 'ثالثة ابتدائي', 'رابعة ابتدائي', 'خامسة ابتدائي', 'سادسة ابتدائي',
  'أولي إعدادي', 'ثانية إعدادي', 'ثالثة إعدادي',
  'أولي ثانوي', 'ثانية ثانوي', 'ثالثة ثانوي',
  'اجتماع الشباب', 'اجتماع الشابات',
];

const SECTOR_OPTIONS = [
  'مدارس الأحد', 'اعدادي', 'ثانوي', 'جامعة', 'خريجين',
];
...
async function renderForm(idStr) {
    ...
    <div class="container">
      ...
      <form class="form-card" id="memberForm" novalidate>
        <div class="form-grid">
          ...
          ${selectFieldHTML(DROPDOWN_FIELDS[2], v.stage, STAGE_OPTIONS)}
          ${genderFieldHTML('stageGender', v.stageGender, !!v.stage)}
          ${selectFieldHTML(DROPDOWN_FIELDS[3], v.sector, SECTOR_OPTIONS)}
          ${genderFieldHTML('sectorGender', v.sectorGender, !!v.sector)}
          ${textFieldHTML(DROPDOWN_FIELDS[4], v.class)}
  ...
  function wireGenderToggle(parentId, childId) {
    const parentSelect = document.getElementById(`f_${parentId}`);
    const childWrap = document.getElementById(`${childId}Wrap`);
    const childSelect = document.getElementById(`f_${childId}`);
  ...
  wireGenderToggle('stage', 'stageGender');
  wireGenderToggle('sector', 'sectorGender');
  ...
  document.getElementById('memberForm').addEventListener('submit', async (e) => {
    ...
    const record = {
      ...
      stageGender: document.getElementById('f_stage').value ? (document.getElementById('f_stageGender').value.trim() || null) : null,
      sector: document.getElementById('f_sector').value.trim() || null,
      sectorGender: document.getElementById('f_sector').value ? (document.getElementById('f_sectorGender').value.trim() || null) : null,
