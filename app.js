...
const NAV_SECTIONS = [
  ...
  { key: 'stage', field: 'stage', label: 'الرحلة', icon: 'journey' },
...
const Admin = {
  ...
  async require() {
    ...
    const storedHash = await SettingsDB.get('pinHash');
    if (!storedHash) {
      return this.setupPin();
    }
    return this.promptPin(storedHash);
  },

  setupPin() {
    return new Promise((resolve) => {
      openPinModal({
        title: 'تعيين رمز حماية للإدارة',
        message: 'أول استخدام: من فضلك اختر رمز مكوّن من 4 أرقام على الأقل لحماية إضافة/تعديل البيانات على هذا الجهاز.',
        confirmLabel: 'تعيين وتفعيل',
        onSubmit: async (pin, close) => {
          if (!pin || pin.length < 4) {
            return 'الرمز لازم يكون 4 أرقام على الأقل';
          }
          const hash = await sha256Hex(pin);
          await SettingsDB.set('pinHash', hash);
...
function memberCardHTML(m) {
  const metaParts = [m.neighborhood, m.class || m.sector].filter(Boolean);
  return `
...
async function renderBrowse(key, valueRaw, params) {
      ...
      <div class="container">
        ...
        ${values.length ? `<div class="member-list">${values.map((v) => {
          ...
          return `<a href="#/browse/${key}/${encodeURIComponent(v)}" class="member-card">
            ...
            <span class="member-info"><span class="member-name">${escapeHTML(v)}</span><span class="member-meta">${c} اسم</span></span>
          </a>`;
  ...
  APP_ROOT.innerHTML = `
    <div class="container">
      ...
      <h2 class="section-title">${escapeHTML(value)}</h2>
      ...
      ${classValues.length > 1 ? `
        <div class="chip-row">
          ...
          ${classValues.map((c) => `<a href="#/browse/${key}/${encodeURIComponent(value)}?${qs({ class: c })}" class="chip${subField === c ? ' active' : ''}">${escapeHTML(c)}<span class="count">${members.filter((m) => m.class === c).length}</span></a>`).join('')}
        </div>` : ''}
  ...
  APP_ROOT.innerHTML = `
    <div class="container">
      ...
      <div class="profile-header">
        ...
        <div>
          ...
          <div class="profile-tags">
            ${member.sector ? `<span class="tag">${escapeHTML(member.sector)}</span>` : ''}
            ${member.class ? `<span class="tag">${escapeHTML(member.class)}</span>` : ''}
      ...
      <div class="info-section">
        ...
        <dl class="info-grid">
          <div class="info-item"><dt>رقم الموبايل</dt><dd class="${member.phone1 ? '' : 'muted'}">${fieldOrFallback(member.phone1)}</dd></div>
          <div class="info-item"><dt>رقم الموبايل (2)</dt><dd class="${member.phone2 ? '' : 'muted'}">${fieldOrFallback(member.phone2)}</dd></div>
        </dl>
      </div>
      ...
      <div class="info-section">
        ...
        <dl class="info-grid">
          <div class="info-item"><dt>الرحلة (مرحله)</dt><dd class="${member.stage ? '' : 'muted'}">${fieldOrFallback(member.stage)}</dd></div>
          <div class="info-item"><dt>القطاع</dt><dd class="${member.sector ? '' : 'muted'}">${fieldOrFallback(member.sector)}</dd></div>
          <div class="info-item"><dt>الفصل</dt><dd class="${member.class ? '' : 'muted'}">${fieldOrFallback(member.class)}</dd></div>
        </dl>
      </div>
...
const DROPDOWN_FIELDS = [
  ...
  { field: 'stage', label: 'الرحلة (مرحله)' },
...
async function renderForm(idStr) {
  ...
  for (const df of DROPDOWN_FIELDS) {
    optionsByField[df.field] = await MembersDB.distinctValues(df.field);
  }
    ...
    <div class="container">
      ...
      <form class="form-card" id="memberForm" novalidate>
        <div class="form-grid">
          ...
          ${DROPDOWN_FIELDS.slice(0, 2).map((df) => comboFieldHTML(df, v[df.field], optionsByField[df.field])).join('')}
          ...
          ${DROPDOWN_FIELDS.slice(2).map((df) => comboFieldHTML(df, v[df.field], optionsByField[df.field])).join('')}
  ...
  DROPDOWN_FIELDS.forEach((df) => wireCombo(df.field));
  ...
  document.getElementById('memberForm').addEventListener('submit', async (e) => {
    ...
    const record = {
      ...
      city: getComboValue('city'),
      neighborhood: getComboValue('neighborhood'),
      street: document.getElementById('f_street').value.trim() || null,
      stage: getComboValue('stage'),
      sector: getComboValue('sector'),
...
function comboFieldHTML(df, currentValue, options) {
  return `
    <div class="field">
      <label for="f_${df.field}">${df.label}</label>
      <div class="combo-row" data-combo="${df.field}">
        <select id="f_${df.field}">
          <option value="">— اختر —</option>
          ${options.map((o) => `<option value="${escapeHTML(o)}" ${o === currentValue ? 'selected' : ''}>${escapeHTML(o)}</option>`).join('')}
          ${currentValue && !options.includes(currentValue) ? `<option value="${escapeHTML(currentValue)}" selected>${escapeHTML(currentValue)}</option>` : ''}
        </select>
        <button type="button" class="combo-add-btn" title="إضافة قيمة جديدة">+</button>
      </div>
    </div>`;
}

function wireCombo(field) {
  const wrap = document.querySelector(`[data-combo="${field}"]`);
  const select = wrap.querySelector('select');
  const btn = wrap.querySelector('.combo-add-btn');
  btn.addEventListener('click', () => {
    if (select.tagName === 'SELECT') {
      const input = document.createElement('input');
      input.type = 'text';
      input.id = select.id;
      input.placeholder = 'اكتب قيمة جديدة';
      wrap.replaceChild(input, select);
      btn.textContent = '↩';
      btn.title = 'رجوع لاختيار من القائمة';
      wrap.dataset.mode = 'text';
      input.focus();
    } else {
      const newSelect = document.createElement('select');
      newSelect.id = field === select.id ? select.id : `f_${field}`;
      newSelect.innerHTML = wrap.dataset.originalOptions || '<option value="">— اختر —</option>';
      wrap.replaceChild(newSelect, select);
      btn.textContent = '+';
      btn.title = 'إضافة قيمة جديدة';
    }
  });
  wrap.dataset.originalOptions = select.innerHTML;
}

function getComboValue(field) {
  const wrap = document.querySelector(`[data-combo="${field}"]`);
  const el = wrap.querySelector('select, input');
