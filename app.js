...
const CATEGORY_DEFS = [
  ...
  { label: 'إعدادي', match: (m) => m.sector === 'اعدادي' },
  { label: 'ثانوي', match: (m) => m.sector === 'ثانوي' },
  { label: 'جامعة', match: (m) => m.sector === 'اجتماع الشباب' },
  { label: 'خريجين', match: (m) => m.sector === 'اجتماع الخريجين' },
];
...
async function renderAdminPanel() {
  renderChrome(true);
  const total = await MembersDB.count();
  // Prefetch the export JSON so the share click handler can call
  // navigator.share() synchronously (WebKit drops the user-gesture
  // permission if share() is awaited-to after an async DB read).
  let cachedExportJSON = null;
  function refreshExportCache() {
    MembersDB.exportJSON().then((j) => { cachedExportJSON = j; }).catch(() => { cachedExportJSON = null; });
  }
  refreshExportCache();
  ...
  document.getElementById('shareBtn').addEventListener('click', () => {
    const attemptShare = (json) => {
      if (!navigator.share || !navigator.canShare) {
        showToast('مشاركة الملفات غير مدعومة على هذا المتصفح. استخدم زر "تنزيل نسخة JSON".', 'danger');
        return;
      }
      const stamp = new Date().toISOString().slice(0, 10);
      const filename = `church-members-backup-${stamp}.json`;
      // Some mobile browsers don't whitelist application/json for file
      // sharing; try progressively broader MIME types with the SAME
      // actual .json file/content until one is accepted.
      const mimeCandidates = ['application/json', 'text/plain', ''];
      let shareFile = null;
      for (const mime of mimeCandidates) {
        const file = new File([json], filename, mime ? { type: mime } : {});
        if (navigator.canShare({ files: [file] })) { shareFile = file; break; }
      }
      if (!shareFile) {
        showToast('مشاركة الملفات غير مدعومة على هذا المتصفح. استخدم زر "تنزيل نسخة JSON".', 'danger');
        return;
      ...
      navigator.share({ files: [shareFile] }).catch((err) => {
        if (err && err.name !== 'AbortError') showToast('تعذّرت المشاركة — جرّب زر "تنزيل نسخة JSON" ثم أرفقه يدويًا في واتساب', 'danger');
      });
    };
    if (cachedExportJSON !== null) {
      attemptShare(cachedExportJSON);
    } else {
      // Cache wasn't ready yet (rare) — fetch now. Refresh it for next time too.
      MembersDB.exportJSON().then((j) => { cachedExportJSON = j; attemptShare(j); });
    }
  });
